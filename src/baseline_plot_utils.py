...
def plot_span_timeline(bars_per_participant, recording_length_min,
                       title="Periods", empty_text="none found",
                       participants_per_figure=25, min_bar_min=0.1,
                       bar_colour="#D85A30"):
    """
    Draw one horizontal timeline row per participant showing when a set of
    periods happened and roughly how long they lasted.

    Every row uses the same x axis, which runs the length of the longest
    recording, so both the position and the length of a period can be
    compared across participants. If there are more participants than
    participants_per_figure they are split across several figures, which
    keeps each figure a readable height instead of producing one very tall
    image.
    ...
    title                   : heading for the figure, describing which periods
                              are being drawn
    empty_text              : what to write on the row of a participant who
                              has no periods to show
    ...
    for page_start in range(0, len(participant_ids), participants_per_figure):
        ...
        for row_number, participant_id in enumerate(page_ids):
            ...
            if not bars:
                ax.text(recording_length_min / 2, y_position, empty_text,
        ...
        page_title = title
        if total_pages > 1:
            page_title = f"{title} (page {page_number} of {total_pages})"
        ax.set_title(page_title, fontsize=11, loc="left")
